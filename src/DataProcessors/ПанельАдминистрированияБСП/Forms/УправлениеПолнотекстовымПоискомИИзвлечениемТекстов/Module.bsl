...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		АвтоЗаголовок = Ложь;
		Заголовок = НСтр("ru='Управление полнотекстовым поиском';en='Full text search management';ro='Setare de căutare full-text'");
		Элементы.ОписаниеРаздела.Заголовок =
			НСтр("ru='Включение и отключение полнотекстового поиска, обновление индекса полнотекстового поиска.';en='Enabling and disabling the fulltext search, fulltext search index update'");
	КонецЕсли;
...
"Пожалуйста, подождите.';en='Full text index is being updated..."
"Please, wait.'"));
	
	ОбновитьИндексСервер();
	
	Состояние(НСтр("ru='Обновление полнотекстового индекса завершено.';en='Update of the full-text index is completed.';ro='Actualizarea indexului full-text este finalizată.'"));
КонецПроцедуры
...
"Пожалуйста, подождите.';en='Full-text index is being cleared..."
"Please, wait.';ro='Index full-text este șters ..."
"Vă rog, așteptați.'"));
	
	ОчиститьИндексСервер();
	
	Состояние(НСтр("ru='Очистка полнотекстового индекса завершена.';en='Full-text index clearance is completed.';ro='Full-text index clearance-ul este finalizat.'"));
КонецПроцедуры
...
Процедура Подключаемый_ПриИзмененииРеквизита(Элемент, ОбновлятьИнтерфейс = Истина)
	...
	Если Результат.Свойство("НеУдалосьУстановитьРежимПолнотекстовогоПоиска") Тогда
		...
		Кнопки.Добавить("АктивныеПользователи", НСтр("ru='Активные пользователи';en='Active users';ro='Utilizatori activi'"));
		...
		ИначеЕсли РеквизитПутьКДанным = "ИспользоватьПолнотекстовыйПоиск" Тогда
			...
			Исключение
				ЗаписьЖурналаРегистрации(
					НСтр("ru='Полнотекстовый поиск';en='Full-text search';ro='Cautare full- text'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
	...
	Если РеквизитПутьКДанным = ""
		...
		Если ИспользоватьПолнотекстовыйПоиск = 1 Тогда
			...
			ИначеЕсли ИндексАктуален Тогда
				СтатусИндекса = НСтр("ru='Обновление не требуется';en='Update is not required';ro='Actualizarea nu este necesară'");
			Иначе
				СтатусИндекса = НСтр("ru='Требуется обновление';en='Update is needed';ro='Este nevoie de actualizare'");
			КонецЕсли;
		Иначе
			ДатаАктуальностиИндекса = '00010101';
			ИндексАктуален = Ложь;
			ФлагДоступность = Ложь;
			СтатусИндекса = НСтр("ru='Полнотекстовый поиск отключен';en='Full-text search is disabled';ro='Căutare full-text este dezactivată'");
		КонецЕсли;
